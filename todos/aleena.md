...
- [ ] `GET /sessions` currently returns every session to anyone with the URL, no auth — fine for a hackathon demo among the team, but worth knowing it's not access-controlled if the URL ever leaks beyond the team
- [ ] No rate limiting per-IP (e.g. `express-rate-limit`) — the concurrency cap above covers subprocess exhaustion but not e.g. someone hammering `GET /sessions` itself
- [ ] Socket `sample` events have no payload size cap (the CSV `/replay` endpoint already has a 25MB one)
