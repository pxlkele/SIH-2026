...
- [x] **Frontend deployed (2026-09-04) — CORS locked down.** `web/` is live at `https://beacon-sih.vercel.app`. Set `CORS_ORIGIN=https://beacon-sih.vercel.app,http://localhost:5173` on Railway (comma-separated list, keeps localhost open for dev). Verified directly against the live backend: the real origin gets reflected in `Access-Control-Allow-Origin`, an arbitrary other origin gets nothing back.
  - Note on urgency: the frontend's data-source picker defaults to `replay` (bundled static CSVs) or `live` (on-device TS Kalman filter, ported from this backend's Python) — neither needs this backend at all. `backend` mode (the real Socket.io pipeline) is an optional third mode, not the default demo path. Locked down anyway since the URL now exists and there's no reason to leave it open.
- [ ] **Still genuinely open — tell Palak the backend is live.** Her todo (`todos/palak.md` → Coordination) still explicitly lists "sign off on backend hosting choice with Aleena — confirm URL + whether we need `SESSIONS_API_KEY` for demo day" as unchecked, as of the latest pull. She's independently concluded `SESSIONS_API_KEY` off is "the correct choice" — she just needs the actual confirmation + URL from you: `https://sih-2026-backend-production.up.railway.app`.
- [x] ~~Alias `GET /session/:id/smoothed`~~ — moot. Palak's todo now references the correct plural `/sessions/:id/smoothed` path (was singular before) — resolved on her end somehow, unclear how, but the mismatch is gone either way.

**Found while deploying the CORS lock — a real bug, not a fluke:** the redeploy got stuck 3+ minutes in "Building" and never cut over, even though the container logs showed it had actually started fine. Root cause: Railway sits behind a reverse proxy (sets `X-Forwarded-For`), and Express wasn't told to trust it — `express-rate-limit` couldn't safely identify real client IPs, threw `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` on every request, and very likely bucketed Railway's own repeated health-check pings under one shared identity, blowing through the rate limit and never letting the health check pass. Fixed with `app.set('trust proxy', 1)`. This wasn't just a deploy-timing nuisance — without the fix, every real visitor may have been sharing one rate-limit bucket instead of getting their own. Redeployed with the fix: cut over normally, no more error in the logs, full live test suite re-passed (including a live private-network OSRM round trip, 379 points).
